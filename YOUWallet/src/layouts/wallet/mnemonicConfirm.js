...
class MnemonicConfirm extends BasePureLayout {
  constructor(props) {
    super(props);

    const { wallet, shuffle } = props.navigation.state.params;
    this.state = {
      wallet: wallet,
      shuffle: shuffle,
      shuffleChecked: []
    };
    this.shuffleCheckedOrder = [];
  }
  ...
  getItemBgColor = (index, item) => {
    return this.state.shuffleChecked[index] ? "#ffe894" : "#f5f5f5";
  };

  checkItem = (index, item) => {
    let shuffleChecked = this.state.shuffleChecked;
    if (shuffleChecked[index]) {
      shuffleChecked[index] = false;
      this.shuffleCheckedOrder.pop();
    } else {
      shuffleChecked[index] = true;
      this.shuffleCheckedOrder.push(item);
    }

    this.setState({ shuffleChecked: _.clone(shuffleChecked) });
  };

  create = () => {
    let wallet = this.state.wallet;
    if (
      this.shuffleCheckedOrder.join(" ") === wallet.mnemonic &&
      BipUtils.validateMnemonic(wallet.mnemonic)
    ) {
      let privateKey = BipUtils.createAccountAtIndex(wallet.mnemonic);
            ...
            return (
              <TouchableOpacity
                style={[
                  ...
                  {
                    backgroundColor: this.getItemBgColor(index + key, item)
                  }
                ]}
                key={key}
                activeOpacity={0.8}
                onPress={() => {
                  this.checkItem(index + key, item);
                }}
              >
          ...
          {data.slice(index + range, index + range * 2).map((item, key) => {
            return (
              <TouchableOpacity
                ...
                activeOpacity={0.8}
                onPress={() => {
                  this.checkItem(index + range + key, item);
                }}
              >
                <Text style={[styles.itemText]}>{item}</Text>
              </TouchableOpacity>
          ...
          {data.slice(index + range * 2, index + range * 3).map((item, key) => {
            return (
              <TouchableOpacity
                ...
                activeOpacity={0.8}
                onPress={() => {
                  this.checkItem(index + range * 2 + key, item);
                }}
              >
                <Text style={[styles.itemText]}>{item}</Text>
              </TouchableOpacity>
          ...
          {data.slice(index + range * 3).map((item, key) => {
            return (
              <TouchableOpacity
                ...
                activeOpacity={0.8}
                onPress={() => {
                  this.checkItem(index + range * 3 + key, item);
                }}
              >
                <Text style={[styles.itemText]}>{item}</Text>
              </TouchableOpacity>
  ...
  createUI = () => {
    return (
      ...
      >
        {this.renderItem(this.state.shuffle)}
        <TouchableOpacity
          style={{
            position: "absolute",
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            {this.i18n("wallet.create.mnemonic-confirm")}
          </Text>
          <TouchableOpacity
            activeOpacity={1.0}
            onPress={() => {
          ...
          >
            ...
            >
              {this.i18n("wallet.create.mnemonic-confirm-notice")}
            </Text>
            {this.createUI()}
          </TouchableOpacity>
        </KeyboardAwareScrollView>
...
export default connect(mapStateToProps, mapDispatchToProps)(MnemonicConfirm);
